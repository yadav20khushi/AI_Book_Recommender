...
"""
    ...
    def get_popular_books_by_location(self, location_code, page_no=1, page_size=15, dtl_kdc_code=None):
        ...
        if not self.library_api_key:
            print(f"[DEBUG] [LIBRARY API] No API key provided")
            return []
        ...
        print(f"[DEBUG] [LIBRARY API] Calling URL: {url}")
        print(f"[DEBUG] [LIBRARY API] Parameters: {params}")
        
        try:
            r = requests.get(url, params=params)
            print(f"[DEBUG] [LIBRARY API] Response status: {r.status_code}")
            print(f"[DEBUG] [LIBRARY API] Response URL: {r.url}")
            
            if r.status_code == 200:
                response_data = r.json()
                print(f"[DEBUG] [LIBRARY API] Response data keys: {list(response_data.keys()) if isinstance(response_data, dict) else 'Not a dict'}")
                
                if "response" in response_data:
                    docs = response_data["response"].get("docs", [])
                    print(f"[DEBUG] [LIBRARY API] Docs type: {type(docs)}, Docs length: {len(docs) if isinstance(docs, list) else 'Not a list'}")
                    
                    if isinstance(docs, dict):
                        docs = [docs]
                    elif not isinstance(docs, list):
                        print(f"[DEBUG] [LIBRARY API] Docs is not a list or dict: {docs}")
                        return []
                    ...
                    print(f"[DEBUG] [LIBRARY API] Processed {len(books)} books")
                    return sorted(books, key=lambda x: x["loan_count"], reverse=True)
                else:
                    print(f"[DEBUG] [LIBRARY API] No 'response' key in response_data")
                    print(f"[DEBUG] [LIBRARY API] Full response: {response_data}")
                    return []
            else:
                print(f"[DEBUG] [LIBRARY API] HTTP error: {r.status_code}")
                print(f"[DEBUG] [LIBRARY API] Response text: {r.text}")
                return []
        except Exception as e:
            print(f"[DEBUG] [LIBRARY API] Exception: {e}")
            return []
...
def process_user_message(user_message):
    ...
    """
    print(f"[DEBUG] [PROCESS_USER_MESSAGE] START - Input: {user_message}")
